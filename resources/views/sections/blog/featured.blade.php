  ...
  <div class="max-w-7xl mx-auto px-5 lg:px-8">
    ...
    <a href="{{ $featured['url'] }}" class="grid lg:grid-cols-2 border border-gray-200 rounded-xl overflow-hidden group hover:-translate-y-1 hover:shadow-lg transition-[transform,box-shadow] duration-200">
      ...
      <div class="p-8 bg-white">
        <div class="flex items-center gap-3 mb-4"><span class="text-[0.7rem] font-semibold px-2.5 py-1 rounded-full bg-gray-950 text-white">{{ $featured['category_name'] }}</span><span class="text-xs text-gray-500 flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> {{ $featured['reading_time_label'] }}</span></div>
