    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <a href="{{ $art['url'] }}" class="group bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col hover:-translate-y-1 hover:shadow-lg transition-[transform,box-shadow] duration-200">
        ...
        <div class="p-5 flex flex-col flex-1">
          <div class="flex items-center gap-2 mb-3"><span class="text-[0.65rem] font-semibold px-2 py-0.5 rounded-full bg-gray-950 text-white">{{ $art['category_name'] }}</span><span class="text-xs text-gray-500 flex items-center gap-1"><i data-lucide="clock" class="w-2.5 h-2.5"></i> {{ $art['reading_time_label'] }}</span></div>
          <h3 class="font-grotesk text-[0.9375rem] font-bold leading-snug mb-2 flex-1">{{ $art['title'] }}</h3>
          <div class="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
            <span class="text-xs text-gray-500 flex items-center gap-1"><i data-lucide="calendar" class="w-2.5 h-2.5"></i> {{ $art['published_at_human'] }}</span>
