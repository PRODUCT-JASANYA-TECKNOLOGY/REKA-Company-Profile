<section id="klien" class="py-24 lg:py-32 bg-white">
    <div class="max-w-7xl mx-auto px-5 lg:px-8">
        <div class="text-center mb-16">
            <span
                class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[0.7rem] font-medium tracking-widest uppercase bg-gray-100 text-gray-500 border border-gray-200">Our
                Klient</span>
            <h2 class="font-grotesk text-4xl sm:text-5xl font-bold tracking-[-0.03em] mt-3">Dipercaya oleh
                Berbagai<br><span class="text-gray-300">Bisnis dan Institusi</span></h2>
            <p class="mt-5 text-gray-500 max-w-2xl mx-auto">Kami membantu pengembangan website, aplikasi, dan sistem
                digital untuk berbagai kebutuhan bisnis dan organisasi.</p>
        </div>
            ...
            @if ($industries->isNotEmpty())
                <div class="relative min-w-[220px]">
                    <select id="industry-filter"
                        class="w-full appearance-none bg-white border border-gray-200 rounded-xl px-5 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all cursor-pointer">
        ...
        <div id="clients-grid" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 border-l border-t border-gray-100">
            @forelse($klients as $klient)
                <div class="client-card aspect-[4/3] sm:aspect-[3/2] flex items-center justify-center p-6 border-r border-b border-gray-100 hover:bg-gray-50 transition-colors duration-300 group relative overflow-hidden"
                    ...
                    <img src="{{ $logoUrl }}" alt="{{ $klient->nama }}"
                        class="h-14 sm:h-16 w-auto max-w-[80%] object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500" />
                </div>
            @empty
